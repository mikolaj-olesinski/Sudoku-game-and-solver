...
def displayNumbers(img, numbers, color=(0, 255, 0)):
    """
    Displays predicted numbers onto the original Sudoku puzzle image.

    Args:
    - img (numpy.ndarray): Original Sudoku puzzle image.
    - numbers (list): List of integers representing predicted digits for each cell in the Sudoku puzzle.
    - color (tuple): Color tuple (BGR format) for displaying numbers (default: green).

    Returns:
    - numpy.ndarray: Image with predicted numbers displayed.
    """
...
def drawGrid(img):
    """
    Draws a grid on the Sudoku puzzle image to visualize the efficiency of perspective transformation.

    Args:
    - img (numpy.ndarray): Sudoku puzzle image.

    Returns:
    - numpy.ndarray: Sudoku puzzle image with overlaid grid.
    """
    secW = int(img.shape[1] / 9)
    secH = int(img.shape[0] / 9)
    for i in range(0, 9):
...
def stackImages(imgArray, scale):
    """
    Stacks multiple images either horizontally or vertically for display.

    Args:
    - imgArray (list): List of lists containing images to be stacked.
    - scale (float): Scale factor for resizing images (default: 1.0).
